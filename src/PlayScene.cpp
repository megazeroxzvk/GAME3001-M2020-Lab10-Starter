	...
	if (!m_bDebugKeys[P_KEY])
	{
		if (EventManager::Instance().isKeyDown(SDL_SCANCODE_P))
		{
			...
			if (m_bPatrolMode)
			{
				...
				m_pPlaneSprite->patrolToggle(true);
			}
			else
			{
				m_pPlaneSprite->patrolToggle(false);
...
void PlayScene::m_movePlaneToNextNode()
{
	m_pTargetPathNode = m_pPatrolPath[m_nextPathNodeIndex];

	//(1,0) right or (0,1) down or (-1,0) left or (0,-1) up !
	auto targetVector = Util::normalize(m_pTargetPathNode->getTransform()->position - m_pPlaneSprite->getTransform()->position);

	//right or left
	if(targetVector.x == 1)
	{
		m_pPlaneSprite->setAngle(directions::RIGHT_DIRECTION);
	}
	else if(targetVector.x == -1)
	{
		m_pPlaneSprite->setAngle(directions::LEFT_DIRECTION);
	}

	//up or down
	if(targetVector.y == 1)
	{
		m_pPlaneSprite->setAngle(directions::DOWN_DIRECTION);
	}
	else if(targetVector.y == -1)
	{
		m_pPlaneSprite->setAngle(directions::UP_DIRECTION);
	}
	...
	{
		m_nextPathNodeIndex++;
		if(m_nextPathNodeIndex > m_pPatrolPath.size() - 1)
		{
...
void PlayScene::start()
{
	...
	//Normalize .. gets unit value.. updated above in patrol functon
	/*glm::vec2 nextPosition = m_pPatrolPath[1]->getTransform()->position -
		m_pPlaneSprite->getTransform()->position;
	glm::vec2 normalized = Util::normalize(nextPosition);
	std::cout << "normalized dir (" << normalized.x << ", " << normalized.y <<")" << std::endl;*/
